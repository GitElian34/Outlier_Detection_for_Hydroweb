...
class RegressionTester(BaseTester):
    ...
    def _create_xarray_data_vars(self, y_hat: np.ndarray, y: np.ndarray):
        ...
        for i, var in enumerate(self.cfg.target_variables):
            ...
            n_out = y_hat.shape[2]
            n_targets = len(self.cfg.target_variables)
            n_out_per_target = n_out // n_targets
            start = i * n_out_per_target
            if n_out_per_target == 1:
                data[f"{var}_sim"] = (('date', 'time_step'), y_hat[:, :, start])
            else:
                quantiles = getattr(self.cfg, 'quantiles', None)
                q50_idx = n_out_per_target // 2
                for q_idx in range(n_out_per_target):
                    q_name = f"q{int(quantiles[q_idx] * 100):02d}" if quantiles else f"q{q_idx}"
                    data[f"{var}_sim_{q_name}"] = (('date', 'time_step'), y_hat[:, :, start + q_idx])
                # alias Q50 -> water_level_sim pour compatibilité avec le reste de NeuralHydrology
                # (calcul des métriques NSE/KGE/FHV, plots, etc.)
                data[f"{var}_sim"] = (('date', 'time_step'), y_hat[:, :, start + q50_idx])
        return data

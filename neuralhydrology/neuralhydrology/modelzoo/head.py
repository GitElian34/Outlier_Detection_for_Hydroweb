...
def get_head(cfg: Config, n_in: int, n_out: int) -> nn.Module:
    ...
    elif cfg.head.lower() == "quantileregression":
        head = QuantileRegression(n_in=n_in, n_out=n_out, n_quantiles=len(cfg.quantiles))
    elif cfg.head.lower() == "":
...
class QuantileRegression(nn.Module):
    def __init__(self, n_in: int, n_out: int, n_quantiles: int):
        super(QuantileRegression, self).__init__()
        self.net = nn.Linear(n_in, n_out * n_quantiles)  # 1 * 5 = 5 sorties

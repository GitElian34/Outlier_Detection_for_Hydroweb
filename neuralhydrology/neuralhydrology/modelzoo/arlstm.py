...
class ARLSTM(BaseModel):
    ...
    def forward(self,
        ...
        ar_size = self._num_ar_inputs if self.cfg.head.lower() == "quantileregression" else self.output_size
        last_prediction = x_d.new_zeros((self._ar_shift, batch_size, ar_size))
        ...
        for x_t in x_d:
            ...
            if self.cfg.head.lower() == "quantileregression":
                q50_idx = len(self.cfg.quantiles) // 2
                last_prediction[0] = prediction[:, q50_idx:q50_idx + 1]
            else:

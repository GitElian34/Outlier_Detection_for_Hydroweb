...
class MaskedKGELoss(BaseLoss):
    """Kling-Gupta Efficiency loss, SANS terme beta, calculée sur le BATCH ENTIER.

    ⚠ Approximation nécessaire : avec predict_last_n=1 (config standard de ce
    projet), chaque échantillon d'entraînement ne contient qu'1 seul pas de
    temps -> impossible de calculer une corrélation ou un ratio de variance
    À L'INTÉRIEUR d'un échantillon (il faut au moins 2 points). La statistique
    est donc calculée sur l'ensemble du batch aplati (tous bassins/temps
    confondus), pas par bassin individuel comme le "vrai" KGE d'évaluation.
    C'est un proxy d'entraînement, pas une KGE au sens strict -- la KGE réelle
    par bassin reste calculée normalement en validation (evaluation/metrics.py).
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor],
                  ground_truth: Dict[str, torch.Tensor], **kwargs):
        mask = ~torch.isnan(ground_truth['y'])
        y = ground_truth['y'][mask]
        y_hat = prediction['y_hat'][mask]

        if y.numel() < 2:
            return torch.tensor(0.0, device=prediction['y_hat'].device, requires_grad=True)

        y_std = y.std() + self.eps
        y_hat_std = y_hat.std() + self.eps

        y_centered = y - y.mean()
        y_hat_centered = y_hat - y_hat.mean()
        r = (y_centered * y_hat_centered).sum() / (
            torch.sqrt((y_centered**2).sum()) * torch.sqrt((y_hat_centered**2).sum()) + self.eps
        )
        alpha = y_hat_std / y_std

        kge = 1 - torch.sqrt((r - 1)**2 + (alpha - 1)**2)
        return 1 - kge
...
class MaskedMultiQuantileLoss(BaseLoss):
    """Pinball loss sur plusieurs quantiles simultanément.
    Utile pour détection d'outlier (point hors [Q05,Q95] -> suspect)
    et complétion avec incertitude (Q50 comme valeur centrale).
    Config YAML : loss: MultiQuantileLoss
                  quantiles: [0.05, 0.25, 0.50, 0.75, 0.95]
    """
    def __init__(self, cfg: Config):
        self.quantiles = cfg.quantiles
        super(MaskedMultiQuantileLoss, self).__init__(
            cfg,
            prediction_keys=['y_hat'],
            ground_truth_keys=['y'],
            output_size_per_target=len(self.quantiles)
        )
    ...
    def _get_loss(self, prediction: Dict[str, torch.Tensor],
        ...
        # y : [n_valid] (masqué sur la dim 1 de gt)
        y = ground_truth['y'][mask]  # [n_valid]

        # y_hat : [n_valid * n_q] -> reshape en [n_valid, n_q]
        mask_expanded = mask.expand_as(prediction['y_hat'])  # [batch, seq, n_q]
        y_hat = prediction['y_hat'][mask_expanded].view(-1, len(self.quantiles))  # [n_valid, n_q]

        # expand y pour matcher n_q
        y = y.unsqueeze(1).expand_as(y_hat)  # [n_valid, n_q]

        quantile_tensor = torch.tensor(
            self.quantiles, dtype=y_hat.dtype, device=y_hat.device
        )
        errors = y - y_hat
        loss = torch.max(quantile_tensor * errors,
                         (quantile_tensor - 1.0) * errors)
        return loss.mean()

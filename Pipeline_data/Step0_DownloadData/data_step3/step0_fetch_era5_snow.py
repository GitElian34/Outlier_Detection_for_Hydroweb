...
"""
═══════════════════════════════════════════════════════════════════════════
step0_fetch_era5_snow.py — Téléchargement ERA5-Land (snow_depth + snowmelt)
═══════════════════════════════════════════════════════════════════════════

Télécharge, mois par mois, snow_depth (instantané, daily-statistics) et
snowmelt (cumulé, décalage 00:00), fusionnés dans
{ERA5_BASE}/Snow/{annee}/{mois}/data_0.nc — lu ensuite par
step3c_era5_snow.py.

Même adaptation que step0_fetch_era5_meteo.py : paramètres au lieu de
constantes, config lue depuis config_step3.py.

Prérequis : pip install cdsapi xarray pandas

Usage :
    python step0_fetch_era5_snow.py --db ./ta_bdd.db
    python step0_fetch_era5_snow.py --bbox 52 -6 40 10
═══════════════════════════════════════════════════════════════════════════
"""
...
log = logging.getLogger("step0_era5_snow")
...
def resolve_netcdf(path: str, extract_dir: str) -> str:
    """Idem step0_fetch_era5_meteo.py : gère le cas zip vs netcdf direct."""
...
def fetch_era5_snow(
    ...
    """
    Télécharge snow_depth + snowmelt pour la période et le bbox donnés,
    mois par mois, avec cache fichier (skip si déjà téléchargé).

    Args: voir fetch_era5_meteo (mêmes paramètres).

    Returns:
        {"downloaded": n, "errors": n}
    """
    ...
    def fetch_hourly_00(year: str, m: str, days: list[str], variables: list[str], out_path: str) -> str:
        ...
        extract_dir = os.path.join(usable_dir, year, m, "snow_tmp")
    ...
    def download_snowmelt(year: str, m: str) -> str:
        ...
        tmp_out = os.path.join(raw_dir, f'_tmp_smlt_daily_{year}_{m}.nc')
    ...
    def download_snow_depth(year: str, m: str) -> str:
        jours = [f"{i:02d}" for i in range(1, 32)]
        tmp_out = os.path.join(raw_dir, f'_tmp_sd_daily_{year}_{m}.nc')
    ...
    def download_snow(year: str, m: str) -> str | None:
        daily_path = os.path.join(raw_dir, f'snow_{year}_{m}.nc')
        if os.path.exists(daily_path):
            log.info(f"  Déjà téléchargé : snow {year}-{m}")
            return daily_path

        log.info(f"Téléchargement snow {year}-{m} (snow_depth + snowmelt)...")
        try:
            sd_nc_path = download_snow_depth(year, m)
            smlt_nc_path = download_snowmelt(year, m)
            ...
            ds_sd.close(); ds_smlt.close(); ds.close()
            ...
            log.error(f"  ❌ Erreur snow {year}-{m} : {e}")
            return None

    for year, m in _year_months(start_date, end_date):
        result = download_snow(year, m)
        if result:
            downloaded += 1
        else:
            errors += 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Étape 0 (pt3) — Téléchargement ERA5-Land snow_depth + snowmelt",
    )

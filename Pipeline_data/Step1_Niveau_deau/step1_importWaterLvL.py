...
Usage standalone :
    python step1_import_hydroweb.py
...
⚠️ CHANGEMENT (config centrale) :
    "dossier" et "--db" ont maintenant des défauts venant de
    config_step1.py (HW_RAW_OUTPUT_DIR, HW_DB_PATH) — le même dossier
    que step0_fetch_hydroweb_next_raw.py remplit, et la même BDD que tu
    utilises habituellement. "dossier" reste overridable en argument
    positionnel comme avant, juste devenu optionnel (défaut au lieu
    d'obligatoire).
═══════════════════════════════════════════════════════════════════════════
"""
...
from Pipeline_data.Step1_Niveau_deau.Parser import parse_hydroweb_directory
from Pipeline_data.Step1_Niveau_deau.config_step1 import (
    HW_RAW_OUTPUT_DIR as DEFAULT_DOSSIER,
    HW_DB_PATH as DEFAULT_DB_PATH,
)
...
def run_step1(dossier: str = DEFAULT_DOSSIER, db_path: str = DEFAULT_DB_PATH,
...
Exemples :
  ...
  python step1_import_hydroweb.py ./data/stations_hw/ --db ./data/test.db --reset
    ...
    parser.add_argument("dossier", type=str, nargs="?", default=DEFAULT_DOSSIER,
                        help=f"Dossier avec les .txt HydroWeb (défaut: {DEFAULT_DOSSIER})")

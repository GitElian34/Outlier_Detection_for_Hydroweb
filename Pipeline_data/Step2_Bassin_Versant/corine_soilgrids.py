...
⚠️ CHANGEMENT (config centrale) :
    DEFAULT_CORINE_PATH / DEFAULT_SOILGRIDS_DIR viennent maintenant de
    config_step2.py (source unique). L'ancien défaut local de ce fichier
    pointait vers un sous-chemin différent
    (u2018_clc2018_v2020_20u1_raster100m/DATA/...) de celui utilisé par
    l'orchestrateur step2_run_all.py — ils divergeaient silencieusement
    en usage standalone vs pipeline complète.
    Si TON fichier Corine est réellement dans ce sous-dossier imbriqué,
    passe-le explicitement via --corine (CLI) ou corine_path=... (Python),
    ou ajuste CORINE_PATH dans config_step2.py.
═══════════════════════════════════════════════════════════════════════════
"""
...
def run_step2d(conn: sqlite3.Connection,
    ...
    if not os.path.exists(corine_path):
        log.error(f"Corine introuvable : {corine_path}")
        return {"updated": 0, "errors": 0}

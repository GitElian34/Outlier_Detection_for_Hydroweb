...
⚠️ CHANGEMENT (config centrale) :
    ROE_DIR vient maintenant de config_step2.py, avec un chemin relatif
    "./data/Step2/Barrage/dataset/" (cohérent avec tous les autres
    chemins de la pipeline, relatifs au répertoire de travail courant).
    L'ancien défaut "../data/Step2/Barrage/dataset/" (avec le "../")
    dépendait du dossier depuis lequel le script était lancé et a déjà
    causé une confusion (fichier introuvable après un simple changement
    de répertoire de travail).
    L'orchestrateur step2_run_all.py passe maintenant explicitement ce
    chemin à run_step2e (avant, aucun roe_dir n'était transmis — le
    défaut interne de ce fichier était utilisé silencieusement, sans
    possibilité de l'override depuis run_step2(**kwargs)).
═══════════════════════════════════════════════════════════════════════════
"""
...
from Pipeline_data.Step2_Bassin_Versant.config_step2 import ROE_DIR as DEFAULT_ROE_DIR
...
def run_step2e(conn: sqlite3.Connection,
               roe_dir: Path = DEFAULT_ROE_DIR) -> dict:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--roe-dir", type=str, default=DEFAULT_ROE_DIR)
    args = parser.parse_args()

    conn = sqlite3.connect(args.db)
    run_step2e(conn, roe_dir=Path(args.roe_dir))
    conn.close()
...
⚠️ CHANGEMENT (config centrale) :
    DEFAULT_DIR_PATH / DEFAULT_ACC_PATH viennent maintenant de
    config_step2.py (source unique), au lieu d'un chemin absolu propre
    à ce fichier qui divergeait de celui utilisé par l'orchestrateur
    step2_run_all.py. Comportement inchangé si tu ne passes pas
    dir_path/acc_path explicitement — seule la provenance du défaut a
    changé, pas sa valeur logique (c'est désormais la même partout).
═══════════════════════════════════════════════════════════════════════════
"""

...
Prérequis :
    ...
    gdaldem (GDAL CLI — ex: apt install gdal-bin)
    Fichier : srtm_france.tif

⚠️ CHANGEMENT (config centrale) :
    DEFAULT_DEM_PATH / DEFAULT_SLOPE_PATH viennent maintenant de
    config_step2.py (source unique). Valeurs inchangées (elles étaient
    déjà cohérentes avec l'orchestrateur), seule la provenance change.
═══════════════════════════════════════════════════════════════════════════
"""

...
# ═══════════════════════════════════════════════════════════════════════
# ERA5_TRANSFERT — pixels Step3_ERA5 par BV
# ═══════════════════════════════════════════════════════════════════════

def insert_era5_pixels(conn: sqlite3.Connection, station_code: str,
                       pixels: list[tuple]) -> int:
    """
    Insère les pixels Step3_ERA5 d'un BV.
...
def get_era5_pixels(conn: sqlite3.Connection, station_code: str) -> list[tuple]:
    """Retourne les pixels Step3_ERA5 d'une station [(lon, lat), ...]."""
    rows = conn.execute(
...
def get_stations_without_era5_pixels(conn: sqlite3.Connection) -> list[str]:
    """Retourne les station_code avec BV mais sans pixels Step3_ERA5."""
    rows = conn.execute("""
...
# ═══════════════════════════════════════════════════════════════════════
# ERA5_BV_JOUR — Step3_ERA5 quotidien agrégé sur BV
# ═══════════════════════════════════════════════════════════════════════
...
def get_era5_bv_jour(conn: sqlite3.Connection, station_code: str):
    """Récupère la série Step3_ERA5 quotidienne complète d'une station."""
    import pandas as pd
    df = pd.read_sql("""

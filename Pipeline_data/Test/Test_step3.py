...
"""
═══════════════════════════════════════════════════════════════════════════
Test_step3.py — Test de l'étape 3 (Step3_ERA5 sur BV + features)
═══════════════════════════════════════════════════════════════════════════
...
Usage :
    ...
    python Test_step3.py ./data/test.db --era5 ./data/Step3_ERA5/usable_data_LAND_France
═══════════════════════════════════════════════════════════════════════════
"""
...
def test_3a_pixels(conn) -> bool:
    """Test 3a : pixels Step3_ERA5 calculés pour chaque BV."""
    print("\n" + "─" * 60)
    print("  TEST 3a — Pixels Step3_ERA5 par BV")
    print("─" * 60)
...
def test_3b_meteo(conn) -> bool:
    """Test 3b : Step3_ERA5 météo quotidien rempli."""
    print("\n" + "─" * 60)
    print("  TEST 3b — Step3_ERA5 météo quotidien")
    print("─" * 60)
...
def test_3c_snow(conn) -> bool:
    ...
    print("\n" + "─" * 60)
    print("  TEST 3c — Step3_ERA5 neige")
    print("─" * 60)
...
def test_coherence(conn) -> bool:
    ...
    print("\n" + "─" * 60)
    print("  COHÉRENCE Step3_ERA5 ↔ FEATURES")
    print("─" * 60)

    # Vérifier que les features correspondent aux données Step3_ERA5
    check = conn.execute("""
        SELECT ma.station_code, ma.measure_date,
    ...
    if check:
        ...
        for code, date, j0, era5 in check:
            ...
            if not match:
                all_match = False
            print(f"    {status} {code} {date} : J0={j0:.3f}  Step3_ERA5={era5:.3f}")
...
def main():
    ...
    print("═" * 60)
    print("  TEST ÉTAPE 3 — Step3_ERA5 sur BV + Features")
    print(f"  BDD  : {db_path}")
    print(f"  Step3_ERA5 : {args.era5}")
    print("═" * 60)
    ...
    print("  BILAN")
    print("═" * 60)
    print(f"  3a Pixels Step3_ERA5   : {'✅' if r_pix else '❌'}")
    ...
    if all_ok:
        ...
        print("\n  Des sous-étapes ont échoué — vérifie les données Step3_ERA5")
        sys.exit(1)

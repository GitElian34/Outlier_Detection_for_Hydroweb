...
"""
═══════════════════════════════════════════════════════════════════════════
step4_create_dataset_insitu.py — Étape 4 (in-situ) : BDD → fichiers .nc
═══════════════════════════════════════════════════════════════════════════

Version in-situ de step4_create_dataset_dahiti.py — même structure, mais
SANS la logique de sous-dossiers par fréquence satellite (27j/10j/21j) :
les stations in-situ ont des mesures journalières, un seul dossier de
sortie suffit.

Génère :
  - Un fichier .nc par station (variables dynamiques) dans time_series/
  - attributes/attributes.csv (attributs statiques pour NeuralHydrology)
  - stations_insitu.txt (liste des IDs pour le yaml)
...
Usage :
    python step4_create_dataset_insitu.py
    python step4_create_dataset_insitu.py --db ./database/insitu_data.db
    python step4_create_dataset_insitu.py --reset
═══════════════════════════════════════════════════════════════════════════
"""
...
log = logging.getLogger("step4_insitu")

# ═══════════════════════════════════════════════════════════════
# PARAMÈTRES GLOBAUX
# ═══════════════════════════════════════════════════════════════
DB_PATH     = "./database/insitu_data.db"
OUTPUT_DIR  = "./data/IA/NeuralHydroDtoD0"
DATE_DEB    = "2016-01-01"
DATE_FIN    = "2025-12-31"
RESET       = False
CLIM_WINDOW = 20
...
# Valeur par défaut si absente de la BDD → 0.0 (mieux que NaN pour l'inférence)
STATIC_COLS = [
    "aire_km2", "lon", "lat",
    "frac_urban", "frac_forest", "frac_agriculture",
    "sg_clay_0_30cm", "sg_sand_0_30cm", "sg_silt_0_30cm",
...
def load_stations(conn: sqlite3.Connection) -> pd.DataFrame:
    ...
    Les colonnes manquantes sont remplies à 0.0.
    """
    df = pd.read_sql("""
        SELECT
            s.station_code,
...
def load_wse_station(conn: sqlite3.Connection,
                     ...
                     date_range: pd.DatetimeIndex) -> pd.DataFrame:
    """
    Charge le niveau d'eau (water level) de la station. Pour l'in-situ,
    orthometric_height contient déjà la médiane journalière (calculée à
    l'étape 1 in-situ), donc une valeur par jour au plus — pas de
    dédoublonnage complexe nécessaire, mais on le garde par sécurité.
    """
    df = pd.read_sql("""
        SELECT measure_date,
    ...
    df = df.set_index("measure_date").groupby(level=0).first()  # dédoublonnage sécurité
...
# EXPORT .NC
# ═══════════════════════════════════════════════════════════════
def write_nc(df: pd.DataFrame, nc_path: str, station_code: str):
    data_vars = {
...
# ═══════════════════════════════════════════════════════════════
def write_attributes(rows: list[dict], output_dir: str):
    """
    Écrit attributes/attributes.csv dans output_dir/.
    ...
    attrs_dir = os.path.join(output_dir, "attributes")
...
def write_stations_txt(station_ids: list[str], output_dir: str):
    """
    Écrit stations_insitu.txt dans output_dir/ pour pointer depuis le yaml.
    """
    if not station_ids:
        return
    path = os.path.join(output_dir, "stations_insitu.txt")
...
# FONCTION PRINCIPALE
# ═══════════════════════════════════════════════════════════════
def run_step4_insitu(db_path: str    = DB_PATH,
    ...
    # Créer / reset le dossier
    ts_path = os.path.join(output_dir, "time_series")
    if reset and os.path.exists(ts_path):
        shutil.rmtree(ts_path)
        log.info(f"Dossier supprimé : {ts_path}")
    ...
    for i, row in stations.iterrows():
        ...
        nc_path = os.path.join(ts_path, f"{station_code}.nc")

        # Attributs statiques — toujours accumulés même si .nc déjà existant
        attr_row = {"station_id": station_code}
        for col in STATIC_COLS:
            attr_row[col] = row.get(col, 0.0)

        if os.path.exists(nc_path) and not reset:
            log.info(f"  [{i+1:3d}/{len(stations)}] {station_code} — .nc existant → skip")
            attrs_rows.append(attr_row)
            station_ids.append(station_code)
            exported += 1
            continue
        ...
        try:
            ...
            if df_era5.empty or df_era5["precipitation"].isna().all():
                log.warning(f"  [{i+1:3d}/{len(stations)}] {station_code} — ERA5 vide → skip")
                skipped += 1
                continue
            ...
            if wse_series.dropna().shape[0] < 5:
                log.warning(f"  [{i+1:3d}/{len(stations)}] {station_code} — WSE vide → skip")
                skipped += 1
                continue

            df_feat = build_features(df_era5, wse_series, date_range)
            if df_feat.empty:
                log.warning(f"  [{i+1:3d}/{len(stations)}] {station_code} — features vides → skip")
                skipped += 1
                continue

            write_nc(df_feat, nc_path, station_code)
            ...
            log.info(
                f"  [{i+1:3d}/{len(stations)}] {station_code:20s} → "
                f"{station_code}.nc | {int(wse_series.dropna().shape[0])} WSE"
            )

        except Exception as e:
            log.error(f"  [{i+1:3d}/{len(stations)}] {station_code} — erreur : {e}")
            errors += 1

    conn.close()

    # ── Écriture attributes.csv + stations txt ──
    write_attributes(attrs_rows, output_dir)
    write_stations_txt(station_ids, output_dir)

    # ── Rapport ──────────────────────────────────────────────────
    print("\n" + "═" * 60)
    print("  RAPPORT ÉTAPE 4 (in-situ)")
    print("═" * 60)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Étape 4 in-situ — BDD → .nc + attributes.csv")

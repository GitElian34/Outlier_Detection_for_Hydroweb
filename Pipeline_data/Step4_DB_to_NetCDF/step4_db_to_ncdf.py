...
"""
═══════════════════════════════════════════════════════════════════════════
step4_create_dataset_hydroweb.py — Étape 4 : BDD HydroWeb Next → fichiers .nc
═══════════════════════════════════════════════════════════════════════════

Génère par sous-dossier (27j / 10j / 21j / autres — selon la fréquence de
passage satellite déduite de la mission) :
  - Un fichier .nc par station (variables dynamiques)
  - attributes/attributes.csv (attributs statiques pour NeuralHydrology)
  - stations_hwnext_<freq>.txt (liste des IDs pour le yaml)
...
Usage :
    python step4_create_dataset_hydroweb.py
    python step4_create_dataset_hydroweb.py --db ./database/hydroweb_next_France.db
    python step4_create_dataset_hydroweb.py --reset

⚠️ Renommé depuis step4_create_dataset_dahiti.py / step4_db_to_ncdf.py —
c'était un copier-coller jamais nettoyé (BDD/dossiers/variables encore
appelés "dahiti" partout alors que ce script tourne sur HydroWeb Next).
Comportement inchangé, seuls les noms ont été corrigés.
═══════════════════════════════════════════════════════════════════════════
"""
...
log = logging.getLogger("step4_hydroweb")

# ═══════════════════════════════════════════════════════════════
# PARAMÈTRES GLOBAUX
# ═══════════════════════════════════════════════════════════════
DB_PATH     = "./database/hydroweb_next_France.db"
OUTPUT_DIR  = "./data/IA/Dataset"
DATE_DEB    = "2016-01-01"
DATE_FIN    = "2025-12-31"
RESET       = False
CLIM_WINDOW = 20
...
# EXPORT .NC
# ═══════════════════════════════════════════════════════════════
def write_nc(df: pd.DataFrame, nc_path: str, station_id: int):
    data_vars = {
...
def write_stations_txt(station_ids: list[str], output_dir: str, subdir: str):
    """
    Écrit stations_hwnext_<subdir>.txt dans output_dir/
    pour pointer depuis le yaml.
    """
    if not station_ids:
        return
    path = os.path.join(output_dir, f"stations_hwnext_{subdir}.txt")
...
# FONCTION PRINCIPALE
# ═══════════════════════════════════════════════════════════════
def run_step4_hydroweb(db_path: str    = DB_PATH,
    ...
    for i, row in stations.iterrows():
        ...
        station_id   = int(station_code.lstrip("0") or 0)
        ...
        nc_path = os.path.join(output_dir, subdir, "time_series", f"{station_id}.nc")

        # Attributs statiques — toujours accumulés même si .nc déjà existant
        attr_row = {"station_id": str(station_id)}
        ...
        if os.path.exists(nc_path) and not reset:
            ...
            stations_by_subdir[subdir].append(str(station_id))
            exported += 1
            continue
        ...
        try:
            ...
            write_nc(df_feat, nc_path, station_id)
            ...
            log.info(
                ...
                f"{station_id}.nc | {int(wse_series.dropna().shape[0])} WSE "
    ...
    # ── Rapport ──────────────────────────────────────────────────
    print("\n" + "═" * 60)
    print("  RAPPORT ÉTAPE 4 (HydroWeb Next)")
    print("═" * 60)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Étape 4 HydroWeb Next — BDD → .nc + attributes.csv")

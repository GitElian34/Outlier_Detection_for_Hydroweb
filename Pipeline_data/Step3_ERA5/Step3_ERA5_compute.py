...
"""
═══════════════════════════════════════════════════════════════════════════
step3_run_all.py — Étape 3 : Step3_ERA5 sur BV + features
═══════════════════════════════════════════════════════════════════════════

Orchestre les 4 sous-étapes :
    3a. Pixels Step3_ERA5 par BV (grille 0.1° dans le polygone)
    3b. Step3_ERA5 météo quotidien (precip/temp/pet) → era5_bv_jour
    3c. Step3_ERA5 neige quotidien (snow_depth/snowmelt) → era5_bv_jour
    3d. Calcul des features → measure_attributes

Usage standalone :
    python step3_run_all.py --db ./data/test.db --era5 ./data/Step3_ERA5/usable_data_LAND_France
...
Usage depuis la pipeline :
    ...
    run_step3(db_path="./data/test.db", era5_base="./data/Step3_ERA5/usable_data_LAND_France")
...
from Pipeline_data.Step3_ERA5.Pixels import run_step3a
from Pipeline_data.Step3_ERA5.Meteo_Temp_Precip_ETP  import run_step3b
from Pipeline_data.Step3_ERA5.Snow import run_step3c
from Pipeline_data.Step3_ERA5.Compute_features import run_step3d
...
def run_step3(db_path: str = "./data/test.db",
    ...
    """
    Étape 3 complète : Step3_ERA5 sur BV + features.

    Args:
        db_path: Chemin vers la BDD SQLite
        era5_base: Chemin vers le dossier Step3_ERA5 usable (obligatoire, pas de
    ...
    if not era5_base:
        raise ValueError(
            "era5_base est obligatoire (ex: ./data/Step3_ERA5/usable_data_LAND_Allemagne). "
    ...
    log.info(f"Dossier Step3_ERA5 utilisé : {era5_base}")

    # ── 3a. Pixels Step3_ERA5 ────────────────────────────────────────
    print("\n" + "═" * 60)
    print("  ÉTAPE 3a — Pixels Step3_ERA5 par BV")
    print("═" * 60)
    try:
    ...
    # ── 3b. Step3_ERA5 météo ─────────────────────────────────────────
    print("\n" + "═" * 60)
    print("  ÉTAPE 3b — Step3_ERA5 météo quotidien")
    print("═" * 60)
    try:
    ...
    # ── 3c. Step3_ERA5 neige ─────────────────────────────────────────
    print("\n" + "═" * 60)
    print("  ÉTAPE 3c — Step3_ERA5 neige quotidien")
    print("═" * 60)
    try:
    ...
    print(f"  Pixels Step3_ERA5          : {n_pixels} ({n_sta_pixels} stations)")
    print(f"  Step3_ERA5 jours (météo)   : {n_era5_days}")
    print(f"  Step3_ERA5 jours (neige)   : {n_snow}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Étape 3 — Step3_ERA5 + features",
        epilog="""
Exemples :
  python step3_run_all.py --db ./data/hydroweb_next_Allemagne.db --era5 ./data/Step3_ERA5/usable_data_LAND_Allemagne
  python step3_run_all.py --db ./data/hydroweb_next_Allemagne.db --reset
    ...
    parser.add_argument("--era5", type=str, default=None,
                        help="Dossier Step3_ERA5 usable (obligatoire sauf avec --reset)")

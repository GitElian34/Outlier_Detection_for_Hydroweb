...
⚠️ CHANGEMENT — récupération automatique du fichier SWORD :
    Le fichier n'est plus attendu directement à SWORD_GLOBAL. À la
    première utilisation, ensure_sword_file() vérifie s'il est déjà
    dans ./data/SWORD/ (dest, léger/rapide à relire ensuite) ; sinon il
    le copie depuis le stockage interne
    (/home/.../Outlier_Detection_DL/data/Step2/Sword/) — même principe
    que les autres copies internes (SRTM, Corine, RiverATLAS, ROE).
...
log = logging.getLogger("sword_connectivity")

# ═══════════════════════════════════════════════════════════════
# PARAMÈTRES
# ═══════════════════════════════════════════════════════════════
SWORD_FILENAME = "global_reaches_SWORD_v16_poly.gpkg"

# Source interne (réseau boîte uniquement) — dépôt d'origine du fichier
SWORD_SOURCE_DIR = Path("/home/sar_hydro/STUDIES/EtudesEB/Outlier_Detection_DL/data/Step2/Sword")

# Destination de travail — utilisée par tous les scripts qui appellent
# load_sword_reaches() (copié ici une seule fois, au premier usage)
SWORD_DEST_DIR = Path("./data/SWORD")
SWORD_GLOBAL = SWORD_DEST_DIR / SWORD_FILENAME
...
# ═══════════════════════════════════════════════════════════════
# RÉCUPÉRATION AUTOMATIQUE DU FICHIER (au premier usage)
# ═══════════════════════════════════════════════════════════════
def ensure_sword_file(
    source_dir: Path = SWORD_SOURCE_DIR,
    dest_dir: Path = SWORD_DEST_DIR,
    filename: str = SWORD_FILENAME,
) -> Path:
    """
    Vérifie que le fichier SWORD est présent dans dest_dir ; sinon le
    copie depuis source_dir (stockage interne). Ne fait rien si déjà
    présent (comparaison par taille, comme les autres copies internes
    du projet).

    Returns:
        Chemin du fichier prêt à l'emploi (dans dest_dir).
    """
    ...
    if not source_path.exists():
        raise FileNotFoundError(
            f"Fichier SWORD introuvable ni dans {dest_dir} ni dans {source_dir}.\n"
            f"  → Ce script ne fonctionne que depuis le réseau interne pour la "
            f"première copie. Dépose {filename} dans {source_dir} si besoin."
        )

    dest_dir_path = Path(dest_dir)
    dest_dir_path.mkdir(parents=True, exist_ok=True)

    log.info(f"Copie du fichier SWORD (première utilisation) : {source_path} → {dest_path}")
...
# ═══════════════════════════════════════════════════════════════
def load_sword_reaches(bbox=FRANCE_BBOX, path=None):
    """
    Args:
        path: si None (défaut), utilise ensure_sword_file() pour
              récupérer/localiser automatiquement le fichier. Passer un
              chemin explicite pour forcer une source différente.
    """
    if path is None:
        path = ensure_sword_file()

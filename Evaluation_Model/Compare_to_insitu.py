...
⚠️ ADAPTÉ AU SCHÉMA UNIFIÉ (measurements/stations), au lieu de l'ancien
schéma (mesures_insitu/h_med_wsh) :
  - get_coords_sat() : déjà compatible (stations/reference_longitude/
    reference_latitude), inchangé.
  - get_insitu_series() : réécrit — lit measurements/orthometric_height
    au lieu de mesures_insitu/h_med_wsh, filtré sur is_valid=1.
...
from Pipeline_data.Step1_Niveau_deau.config_step1 import (
    HW_DB_PATH as DEFAULT_HW_DB,
    INSITU_DB_PATH as DEFAULT_INSITU_DB,
    INSITU_GPKG_PATH as DEFAULT_INSITU_SHP,
)

# Sword_connectivity.py est dans le même dossier que ce script — pas
# besoin de sys.path.insert, l'import direct suffit.
...
HW_DB      = DEFAULT_HW_DB
INSITU_DB  = DEFAULT_INSITU_DB
INSITU_SHP = DEFAULT_INSITU_SHP
...
def get_coords_sat(db_path, code):
    """
    Coordonnées d'une station (schéma unifié : stations/reference_longitude/
    reference_latitude) — inchangé, déjà compatible avec le schéma actuel.
    """
    key = (db_path, code)
...
def get_insitu_series(code_sta):
    """
    Série de niveau d'eau in-situ — schéma unifié (measurements/
    orthometric_height), au lieu de l'ancien mesures_insitu/h_med_wsh.
    Filtré sur is_valid=1, comme pour les autres sources.
    """
